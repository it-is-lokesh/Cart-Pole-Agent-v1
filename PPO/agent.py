...
class Agent(AgentConfig):
    ...
    def new_random_game(self):
        ...
        screen, reward, term, trunc, info = self.env.step(action)
        ...
        while not solved:
            ...
            while not solved:
                ...
                if terminal:
                    ...
                    if(self.prev_len<episode_length):
                        torch.save(self.policy_network.state_dict(), "./best_model.model")
                        np.save("./best_rew_curve.npy", np.array(reward_history))
                        np.save("./best_avg_rew.npy", np.array(avg_reward))
                    self.env.reset()
    ...
    def finish_path(self, length):
        state = torch.FloatTensor(self.memory['state'][-length:]).to(device)
        reward = torch.FloatTensor(self.memory['reward'][-length:]).to(device)
        next_state = torch.FloatTensor(self.memory['next_state'][-length:]).to(device)
        terminal = torch.FloatTensor(self.memory['terminal'][-length:]).to(device)
        ...
        if self.memory['td_target'].shape == torch.Size([1, 0]):
            ...
            print("Ans:", td_target.data.get_device())
